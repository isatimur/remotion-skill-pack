...
MODE="${1:-full}"
TARGET="${HOME}/.claude/skills/remotion-skill-pack"
...
if [ -n "${RSP_CORE_DIR:-}" ] && [ -f "$RSP_CORE_DIR/SKILL.md" ]; then
  ...
  if [ -z "$CORE_PKG_DIR" ]; then
    echo "ERROR: cannot locate @remotion-skill-pack/core. Run via 'npx @remotion-skill-pack/install' or install core globally first." >&2
    exit 1
  fi
...
echo "remotion-skill-pack › claude-code adapter"
...
EOF
  echo "  ✓  MCP server registered in $MCP_CONFIG"
fi

echo ""
echo "Done. Open a new Claude Code session — the remotion-skill-pack skill is now active."
